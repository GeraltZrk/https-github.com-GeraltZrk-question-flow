...
export function AnalysisBadge({ mode }: { mode: string }) {
  const cfg = CONFIG[mode];
  if (!cfg) return null;
  return (
    <span style={{ display:"inline-block", padding:"0.15rem 0.6rem", borderRadius:4, fontSize:"0.7rem", fontWeight:700, letterSpacing:"0.05em", background:cfg.bg, color:cfg.color }}>
      {cfg.label}
    </span>
  );
}
